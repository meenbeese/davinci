...
const getCharacters = async (story: string, model: GenerativeModel) => {
  const SYSTEM_PROMPT = `You must extract the story characters and their attributes from the story. The scenes within the story are delimited by "<scene>". The characters are the main subjects of the story and are usually people, animals, or objects that can talk or interact with each other. Each character should have a name. You must output all characters names with a list of their attributes seperated by commas. Do NOT output anything except the character names and attributes. Be as descriptive as possible for each character. Example input 1: "Whiskers the Cat stretched out her paws hoping that Clark would high five her. <scene> Clark the Dog wagged his tail and barked at Whiskers. <scene> Whiskers was very sad and ran away." Example output 1: '{"Whiskers the Cat" : "Cat, Fluffy, Orange", "Clark the Dog" : "Dog, Loud, Energetic, Black"}' Example input 2: "Noah noticed that the sun was shining brightly in the sky. <scene> The clouds were fluffy and white. <scene> The rain started to fall and hit Noah on the head." Example output 2: '{"Noah" : "Human, Child"}. Input: "${story}"`;
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const characters_response = await getCharacters(story, model);
    const title_response = await getTitle(story, model);

    return res.status(200).json({ characters: characters_response, title: title_response });
  } catch (error) {
